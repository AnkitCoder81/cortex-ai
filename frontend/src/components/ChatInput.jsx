import { Code2, FileText, Globe, Image, ImageIcon, MessageSquare, Mic, MicOff, Paperclip, Presentation, Send, X, Zap } from 'lucide-react'
...
const agents=[
  ...
  return (
   ...
   rounded-2xl px-4 pt-3.5 pb-3'>
    ...
    {selectedFile && 
      <div className='my-3'>
        <div className='inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.
        04] px-3 py-2'>
          {
            ...
            />:selectedFile.type.startsWith("image/") && <img src={URL.createObjectURL
              (selectedFile)} className='h-10 w-10 rounded-xl object-cover mt-3'/>
                  ...
                  <div>
          <p className='text-rs text-white'>
           {selectedFile?.name} 
          </p>
   ...
   <div className='flex items-center justify-between'>
    <div className='flex items-center gap-1'>
      ...
      <button className='flex items-center justify-center w-8 h-8 rounded-lg text-slate-600
       ...
       transition-all duration-150 bg-transparent cursor-pointer' onClick={()=>fileRef.current.click()}>
          <Paperclip size={16}/>
      </button>
      <button 
      onClick={toggleMic}
      ...
      className={`flex items-center justify-center w-8 h-8 rounded-lg transition-all duration-150 cursor-pointer ${listening ? 
        "bg-red-500 text-white":"text-slate-600 hover:bg-white/[0.05]"}`}>
  ...
  value.trim()
    ? "cursor-pointer bg-linear-to-br from-indigo-500 to-violet-700 hover:opacity-90 text-white"
